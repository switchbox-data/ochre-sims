...
CREATE TABLE housing_units AS (
  WITH tmp AS (
    SELECT * FROM read_csv_auto('/workspaces/reports/data/resstock/2022_resstock_amy2018_release_1.1/' || getenv('STATE') || '_baseline_metadata_and_annual_results.csv')
  )
...
CREATE TABLE annuals_results AS (
  WITH u3 AS (
    SELECT * FROM read_csv_auto('/workspaces/reports/data/resstock/2022_resstock_amy2018_release_1.1/' || getenv('STATE') || '_upgrade03_metadata_and_annual_results.csv')
  ),
  u4 AS (
    SELECT * FROM read_csv_auto('/workspaces/reports/data/resstock/2022_resstock_amy2018_release_1.1/' || getenv('STATE') || '_upgrade04_metadata_and_annual_results.csv')
  ),
  baseline AS (
    SELECT * FROM read_csv_auto('/workspaces/reports/data/resstock/2022_resstock_amy2018_release_1.1/' || getenv('STATE') || '_baseline_metadata_and_annual_results.csv')
  ),
